                    ...
                    <#if book.id == record.bookId && record.returnDate?has_content == false>
                        <#list users as u>
                            <#if u.id == record.userId>
                                <div class="text-muted">Выдано: ${u.firstName} ${u.lastName!}
                                    <#if user?? && u.id == user.id>
                                    <form action="/books" class="ms-3" method="post" style="display:inline;">
                                        <input type="hidden" name="action" value="return">
                                        <input type="hidden" name="bookId" value="${book.id}">
                                        <button class="btn btn-outline-danger">Вернуть</button>
                                    </form>
                                    </#if>
                                </div>
                            </#if>
            ...
            <#else> <#if user??>
                        <#if user.authorized>
                            <form action="/books" class="ms-3" method="post" style="display:inline;">
                                ...
                                <input type="hidden" name="bookId" value="${book.id}">
                                <button type="submit" class="btn btn-outline-success">Взять</button>
                            </form>
                        </#if>
                    </#if>
            </#if>
